from flask import (
    render_template, request, flash, redirect, session, url_for
)
from bank import app, db
from bank.model import (
    UserRegistration, Customer, State, City, Account, AccountStatus, CustomerStatus, TransactionControl
)
from bank.functions import (
    get_formatted_date, add_transaction, account_type_list
)
import datetime
...
def customer(update_id):     
    ...
    if update_id:
        ...
        postData = {
            ...
            'customer_ssn_id': customer_obj.ssn_id,
            'age': customer_obj.age,
            'address': customer_obj.address,
            'state_id': customer_obj.state,
            'city_id': customer_obj.city
        }
...
def customer_status():
    ...
    customer_status = Customer.query.outerjoin(CustomerStatus).add_columns(
        Customer.id,Customer.name,Customer.ssn_id,CustomerStatus.message,
        CustomerStatus.status,CustomerStatus.updated_at
        ).order_by(Customer.id.asc(),CustomerStatus.updated_at.asc()).all()
...
def ajax_delete_customer():
    ...
    return {'status' : True,'url' : url_for('customer_status')}
...
def transaction_control():
    ...
    return {'url' : url_for('account_view_profile')+'/'+str(source_account_id),'status' : status}        
